...
class ArticleController extends CommonController{
	...
	public function sendArticle(){
		...
		$data['art_author'] = session('team_id'); //作者为协会ID
		...
		//文章类
			...
			$data['art_type_id'] = 2;//2为文章杂烩，社团的所有文章类资讯都归到这一分类
		}
	...
	public function articleList(){
		...
		$condition['art_author'] = session('team_id');
	...
	public function edit(){
		...
		if(is_numeric($art_id)){
			...
			//是否有权限修改
			if($this->article['art_author']!=session('team_id')){
				echo "<script>alert('you had not the permission');history.go(-1);</script>";
				exit();
			}
	...
	public function updateArticle(){
		...
		M('article')->save($data);
		$this->redirect('Home/Article/articleList');
	}
